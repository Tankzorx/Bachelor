...
PassportConfigurator.prototype.configureProvider = function (name, options) {
  ...
  var resourceOwnerPassword = options.resourceOwnerPassword;
  ...
  } else if (resourceOwnerPassword){
    self.app.post(authPath, passport.authenticate(name, _.defaults({scope: scope, session: session, clientId: clientID, clientSecret: clientSecret}, options.authOptions)));
  } else {
  ...
  if (link) {
    ...
    var customCallback = options.customCallback || function (req, res, next) {
      ...
      passport.authenticate(name, _.defaults({session: session,scope: scope}, options.authOptions), function (err, user, info) {
        if (err) {
          return next(err);
        }
        if (!user) {
